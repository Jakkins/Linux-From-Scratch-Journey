...
> Should I configure (./configure) gmp, mpfr and mpc?
...
**Reading the file contrib/download_prerequisites, the files: gmp, mpfr, mpc have to go inside gcc-9.2.0/**
...
10 | #include <isl/schedule.h>
   |          ^~~~~~~~~~~~~~~~
```
And by the fact that conftest is just a test and that I don't need ISL, I'll just goes.
```bash
make install
...
I noticed something
```bash
# This is not the problem for now
cd build/
../configure bla bla
...
```
> When you have problem with configure the proper config.log will provide the hint on what is going wrong
...
So it appears that gcc can't find **mpc** lib

> checking for suffix of object files... configure: error: in `/lfs/sources/gcc-9.2.0/build/x86_64-lfs-linux-gnu/libgcc
...
> --prefix=$LFS_T. Maybe gmp, mpfr, mpc should be inside /lfs/tools/
