
wd719x -> Driver for Western Digital WD7193, WD7197 and WD7296 SCSI cards
aic94xx -> Adaptec SAS 44300, 48300, 58300 Sequencer Firmware for AIC94xx driver	

https://support.mozilla.org/en-US/kb/fix-common-audio-and-video-issues
	ci sta qualcosa con PulseAudio

### localtime possible conf indication

Emulator: ### WARNING: could not find /etc/localtime or /usr/share/zoneinfo/localtime. unable to determine host timezone

### Datas

Example
```bash
sudo pacman -S chromium
sudo pacman -R chromium
```
Still data of chromium on /.config/chromium/